...
class CCoureur(threading.Thread):
    ...
    while len(dormeurs) > 0:
        for i in range(len(dormeurs)):
            if not dormeurs[i].is_alive():
                text = "{0} : {1} se lève après {2:.3f} ({3:.3f})".format(
                    timeLocal(), dormeurs[i].nom, dormeurs[i].elapsedTime, dormeurs[i].sleepyTime
                )
